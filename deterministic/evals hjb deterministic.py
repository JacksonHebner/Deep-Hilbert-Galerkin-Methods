...
model.load_state_dict(torch.load("Critic, trained control, no noise, 25 func, 250 hilbert, 600 neur one layer, WN stat QPDE.pt2", weights_only=True, map_location=torch.device('cpu')))
model.eval()
model_actor = SimpleNN(dim_in=dim_in, num_neurons=600, num_layers=1, dim_out=dim_in)
model_actor.load_state_dict(torch.load("Actor, trained control, no noise, 25 func, 250 hilbert, 600 neur one layer, WN stat QPDE.pt2", weights_only=True, map_location=torch.device('cpu')))
model_actor.eval()

#no noise
sigma = 0

#target measure is WN
