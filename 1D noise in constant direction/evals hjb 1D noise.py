...
model.load_state_dict(torch.load("Critic, trained control, sigma = 1sqrt(2pi), 25 func, 250 hilbert, 600 neur one layer, WN stat.pt2", weights_only=True, map_location=torch.device('cpu')))
model.eval()
model_actor = SimpleNN(dim_in=dim_in, num_neurons=600, num_layers=1, dim_out=dim_in)
model_actor.load_state_dict(torch.load("Actor, trained control, sigma = 1sqrt(2pi), 25 func, 250 hilbert, 600 neur one layer, WN stat.pt2", weights_only=True, map_location=torch.device('cpu')))
model_actor.eval()

#for sigma = 1/sqrt(2pi)
k_int = torch.arange(1, dim_cutoff_theoretical+1)
odd = (k_int % 2 == 1)
const_proj_full = (4.0/torch.pi) * odd / k_int / math.sqrt(2)
sigma = const_proj_full
