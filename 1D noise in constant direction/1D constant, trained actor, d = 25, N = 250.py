...
const_proj = const_proj_full[0:basis_size]
...
for j in range(0, epochs):
    if (j % 2 == 0):
        ...
        pde_loss = (pde_op_vec(qnet, control_net, grid)**2).mean()
...
torch.save(qnet.state_dict(), "Critic, trained control, sigma = 1sqrt(2pi), 25 func, 250 hilbert, 600 neur one layer, WN stat.pt2")
torch.save(control_net.state_dict(), "Actor, trained control, sigma = 1sqrt(2pi), 25 func, 250 hilbert, 600 neur one layer, WN stat.pt2")
